...
public class CompressionSamples : IIntegrationService
{
    ...
    public async Task RunAsync()
    {
        ...
        await PostAndReadPosterWithWithGZipCompressionAsync();
    }
    ...
    private async Task PostAndReadPosterWithWithGZipCompressionAsync()
    {
        var httpClient = _httpClientFactory.CreateClient("MoviesAPIClient");
        ...
        using (var memoryContentStream = new MemoryStream())
        {
                ...
                "api/movies/d8663e5e-7494-4f81-8739-6e0de1bea7ee/posters/"))
            {
                ...
                {
                    using (var gzipStream = new GZipStream(compressedMemoryContentStream, CompressionMode.Compress))
                    {
                        ...
                        using (var streamContent = new StreamContent(compressedMemoryContentStream))
                        {
                            ...
                            var stream = await response.Content.ReadAsStreamAsync();
                            var poster = JsonSerializer.DeserializeAsync<Poster>(stream,
                                _jsonSerializerOptionsWrapper.Options);
